...
module Kitchen
  class SSH
    ...
    define_method(:upload!) do |local, remote, options = {}, &progress|
      mode = ENV['TRANSFER_MODE'] || 'sftp'
      start_time = Time.now
      case mode
      when 'rsync'
        rsync_upload(local, remote, options, &progress)
      when 'scp'
        old_upload.bind(self).call(local, remote, options, &progress)
      when 'sftp'
        sftp_upload(local, remote, options, &progress)
      end
      logger.info("[#{mode}] Time taken to upload #{local} to #{username}@#{hostname}:#{remote}: " +
